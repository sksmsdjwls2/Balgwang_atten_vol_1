...
class AttendanceSystem:
    def __init__(self):
        ...
        self.initialize_practice_dates_file()
    ...
    def initialize_practice_dates_file(self):
        if not os.path.exists(self.practice_dates_file):
            with open(self.practice_dates_file, 'w', encoding='utf-8') as f:
                f.write("")

    def get_practice_dates(self):
        """정기 연습 날짜 목록을 가져옴"""
        try:
            with open(self.practice_dates_file, 'r', encoding='utf-8') as f:
                dates = [line.strip() for line in f if line.strip()]
            return dates
        except FileNotFoundError:
            return []

    def add_practice_date(self, date):
        """정기 연습 날짜 추가"""
        dates = self.get_practice_dates()
        if date not in dates:
            ...
            with open(self.practice_dates_file, 'w', encoding='utf-8') as f:
                for d in dates:
                    f.write(f"{d}\n")
            return True, f"{date}가 정기 연습 날짜에 추가되었습니다."
        return False, f"{date}는 이미 등록된 정기 연습 날짜입니다."

    def remove_practice_date(self, date):
        """정기 연습 날짜 삭제"""
        dates = self.get_practice_dates()
        if date in dates:
            dates.remove(date)
            with open(self.practice_dates_file, 'w', encoding='utf-8') as f:
                for d in dates:
                    f.write(f"{d}\n")
            return True, f"{date}가 정기 연습 날짜에서 삭제되었습니다."
        return False, f"{date}는 등록되지 않은 날짜입니다."

    def get_absence_dates(self, name):
        """특정 동아리원의 지각/결석 날짜 조회"""
        ...
        # 해당 동아리원의 지각/결석 기록만 필터링
        member_records = df[df['이름'] == name]
        absence_records = member_records[member_records['출석상태'].isin(['지각', '결석'])]
        
        if len(absence_records) == 0:
            return None, f"{name}님의 지각/결석 기록이 없습니다."
        
        # 날짜별로 정리
        late_dates = absence_records[absence_records['출석상태'] == '지각']['날짜'].tolist()
        absent_dates = absence_records[absence_records['출석상태'] == '결석']['날짜'].tolist()
    ...
    def get_attendance_by_date(self, name, date):
        """특정 날짜의 출석 상태 조회"""
        df = pd.read_excel(self.data_file)
        members = self.get_members_list()
        ...
        record = df[(df['이름'] == name) & (df['날짜'] == date)]
        
        if len(record) == 0:
            return None, f"{date}에 {name}님의 출석 기록이 없습니다."
        
        return record.iloc[0]['출석상태'], None
...
def main():
    ...
    menu = st.sidebar.selectbox(
        "메뉴 선택",
        ["출석 체크", "출석 현황 조회", "날짜별 출석 조회", "연습 진행 현황", "동아리원 관리", "출석 기록 수정", "지각/결석 날짜 조회", "정기 연습 날짜 관리"]
    )
    ...
    if menu == "출석 체크":
    ...
    elif menu == "지각/결석 날짜 조회":
        st.header("지각/결석 날짜 조회")
        ...
        if members:
            ...
            if st.button("지각/결석 날짜 조회"):
                absence_data, error = system.get_absence_dates(selected_member)
                
                if error:
                    st.error(error)
                else:
                    st.subheader(f"{selected_member}님의 지각/결석 기록")
                    ...
                    with col2:
                        st.write("**결석한 날짜:**")
                        if absence_data['결석']:
                            for date in absence_data['결석']:
                                st.write(f"- {date}")
                        else:
                            st.write("결석 기록 없음")
                    
                    # 통계 정보
                    total_late = len(absence_data['지각'])
                    total_absent = len(absence_data['결석'])
                    st.write(f"**총 지각 횟수:** {total_late}회")
                    st.write(f"**총 결석 횟수:** {total_absent}회")
        else:
            st.info("등록된 동아리원이 없습니다.")

    elif menu == "정기 연습 날짜 관리":
        st.header("정기 연습 날짜 관리")
        
        # 현재 등록된 정기 연습 날짜 표시
        practice_dates = system.get_practice_dates()
        if practice_dates:
            st.subheader("현재 등록된 정기 연습 날짜")
            for date in practice_dates:
                st.write(f"- {date}")
        else:
            st.info("등록된 정기 연습 날짜가 없습니다.")
        
        st.subheader("정기 연습 날짜 추가")
        ...
        if st.button("정기 연습 날짜 추가"):
            success, message = system.add_practice_date(new_date.strftime('%Y-%m-%d'))
            if success:
                st.success(message)
            else:
                st.error(message)
        ...
        # 정기 연습 날짜 삭제
        if practice_dates:
            st.subheader("정기 연습 날짜 삭제")
            date_to_remove = st.selectbox(
                "삭제할 정기 연습 날짜를 선택하세요",
                options=practice_dates
            )
            
            if st.button("정기 연습 날짜 삭제"):
                success, message = system.remove_practice_date(date_to_remove)
                if success:
                    st.success(message)
                else:
                    st.error(message)
